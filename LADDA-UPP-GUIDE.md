...
⚠️ **VIKTIGAST:** Ladda upp **ALLA** filer OCH hela **`icons/`-mappen**. Om icons-mappen saknas
får du felet "Fix the links to your icons" i PWABuilder. Använd helst zip-filen så inget tappas bort.

1. Logga in på GitHub (ditt konto `den-tarzan`).
2. Skapa ett nytt repo, t.ex. **`Force-of-Habit`**.
3. Ladda upp **hela innehållet** ur denna mapp:
   - `index.html`, `manifest.webmanifest`, `sw.js`, `privacy.html`
   - **HELA `icons/`-mappen med alla 19 bilder** (detta är det som ofta glöms!)
   - Tips: dra in alla filer + icons-mappen samtidigt, eller packa upp `force-of-habit.zip` och ladda upp allt.
...
👉 **Testa två saker i webbläsaren innan du går vidare:**
   - `https://den-tarzan.github.io/Force-of-Habit/` → appen ska ladda
   - `https://den-tarzan.github.io/Force-of-Habit/icons/icon-512.png` → du ska se FORCE-loggan.
     Får du 404 här saknas icons-mappen — ladda upp den igen.
